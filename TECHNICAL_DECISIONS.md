...
| TD-006 | 每關必須觸發氣體交換，但兩次 QTE 失敗後仍可過關 |
...
每關的主要 Gas Token 是不可略過的縱向觸發區；畫面中的 Token 模型只是視覺提示，不使用 lateral 碰撞決定是否觸發。
...
2. 首次失敗後，在同一微血管後段建立一次重試觸發區。
3. 第二次失敗後設為 `FAILED`，維持交換前血管顏色並允許通關。
4. 成功時設為 `SUCCESS`，移除尚未觸發的重試區並套用交換後顏色。
5. 終點只接受 `SUCCESS` 或 `FAILED`；若仍為 `PENDING`，由終點前的保底觸發區啟動 QTE。
6. 成功或失敗結果顯示 0.8 秒；該期限在 PAUSED 中仍繼續。
