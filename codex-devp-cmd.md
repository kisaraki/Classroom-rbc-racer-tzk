...
| `REBUILD` | 原始碼遺失／損壞，或使用者明確要求從零重建 | 在新的空目錄依 Phase 00～10 建置 |
...
8. package.json、tests/phase-10-audit.mjs、js/config.js、js/data/levels.js
...
- 完整專案已存在時，選 MAINTAIN；先跑 npm run test:phase10，不得重做 Phase 00～10。
...
Phase 10 本機驗收使用 Node.js `v24.15.0`。其他 Node.js 版本能否使用，以 `npm run test:phase10` 的實際結果為準；不得為此加入相依套件或編譯步驟。
...
階段完成的最低測試數是歷史回歸底線，不是為了湊數。重建若用較少但等價的測試，必須解釋覆蓋對應；最終仍必須達到現行 190 tests 與 8 audits。
...
2. Build job 先執行 `npm run test:phase10`。
...
| 症狀 | 常見原因 | 正確處理 |
| --- | --- | --- |
| `file://` 白畫面或 CORS | 直接開啟 HTML | 啟動靜態 HTTP server |
