...
| `REBUILD` | 原始碼遺失／損壞，或使用者明確要求從零重建 | 在新的空目錄依 Phase 00～11 建置 |

預設模式是 `RESTORE`。看到 Phase 指令不代表必須重做；只有明確符合 `REBUILD` 才可從 Phase 00 開始。
...
4. codex-devp-cmd.md
5. README.md
6. PHASE_REPORT_TEMPLATE.md
7. reports/README.md 與現有 Phase 報告
8. package.json、tests/phase-11-audit.mjs、js/config.js、js/data/levels.js
...
- 完整專案已存在時，選 MAINTAIN；先跑 npm run test:phase11，不得重做 Phase 00～11。
- 只有原始碼不可取得／無法修復，或使用者明確要求從零實作時，才選 REBUILD。
- REBUILD 必須在新的空目錄一次只完成一個 Phase，測試、修正、重測並寫報告；
  前一階段未 PASS 不得繼續。
...
Phase 11 必須以目前環境實際執行 `npm run test:phase11`；不得為 Node.js 版本差異加入相依套件或編譯步驟。Phase 10 的 Node.js `v24.15.0` 紀錄僅是歷史環境證據。
...
3. 執行 `npm run test:phase11` 建立變更前基線。
4. 若基線失敗，先判斷是否由既有未提交變更或非 runtime 產物造成，不得直接弱化測試。
5. 只修改必要檔案，補上能重現問題的測試。
6. 執行目標測試，再執行完整 Phase 11 gate。
7. 以實際瀏覽器驗證受影響流程與兩種解析度。
8. 執行 `git diff --check` 與 `git status --short`。
9. 回報變更、理由、測試輸出、未測項目與殘餘風險。

不得把一般維護工作偽裝成新的 Phase 00～11，也不得改寫歷史 PASS 報告。新的修正應建立獨立報告或提交說明。

---

# 五、REBUILD 共通階段閘門

真正從零重建時，每個 Phase 都使用下列指令模板：

```text
請只執行 RBC Racer Phase <NN>。

開始前：
1. 確認 Phase <NN-1> 報告存在且為 PASS；Phase 00 除外。
2. 列出本階段授權範圍、明確排除項目與預計修改檔案。
3. 檢查工作樹，不得覆寫不屬於本階段的既有變更。

執行時：
4. 只實作本階段範圍；所有數值讀取 js/config.js。
5. 為新增契約補上單元或整合測試。
6. 執行目前全部自動測試及適用的瀏覽器驗收。
7. 記錄錯誤、重現步驟與根因，在本階段修正並完整重測。

完成時：
8. 依 PHASE_REPORT_TEMPLATE.md 建立 reports/phase-<NN>-report.md。
9. 報告必須包含範圍、檔案、環境、原始輸出、錯誤、修正、重測與殘餘風險。
10. 只有所有門檻通過才標示 PASS；否則標示 BLOCKED 並停止。
11. 未經使用者要求不得 push；不得把兩個 Phase 合併成一個提交。
```

階段完成的最低測試數是歷史回歸底線，不是為了湊數。重建若用較少但等價的測試，必須解釋覆蓋對應；最終仍必須達到現行 203 tests 與 9 audits。

---

# 六、Phase 00～11 重建矩陣

| Phase | 授權範圍 | 必要結果 | 歷史測試底線 |
| --- | --- | --- | ---: |
...
| 11 | 小地圖交換鎖點、2 倍減益、血球破裂、CO 中毒、Time Out、蒸氣、啟動與可見碰撞輪廓 | 203 tests + 9 audits；`main` 發布前必須完整通過 | 203 + 9 |
...
- 玩家畫面不得顯示 Phase、build、FPS、Pointer Lock、內部 state、checkpoint seed 或 cutscene phase ID；KOSMOS TOOLKIT 與必要操作儀表保留。
- 完成 `npm run test:phase11`、`git diff --check`、語法檢查與適用的桌面瀏覽器驗收。
...
BASELINE_COMMIT=0cdaa69b27c58816144ef58face1d72e99f9a2fd
THREE_RELEASE=r184
...
2. Build job 先執行 `npm run test:phase11`。
...
Phase 報告必須使用 `PHASE_REPORT_TEMPLATE.md`。歷史報告是證據，不可覆寫；若維護工作修正既有 Phase 行為，另建具日期或主題名稱的報告。
