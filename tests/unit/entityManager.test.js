...
import { getEntityType } from "../../js/data/entityTypes.js?v=phase07-status-r2";
import { LEVELS } from "../../js/data/levels.js?v=phase07-status-r2";
import { createPlayerState } from "../../js/data/schemas.js?v=phase07-status-r2";
import {
  EntityManager,
  buildEntityWeightTable,
  sampleEntityOffset
} from "../../js/systems/EntityManager.js?v=phase07-status-r2";
import {
  assert,
  assertApproximately,
  assertDeepEqual,
...
export function registerEntityManagerTests(harness) {
  ...
  harness.test("level multipliers and uniform-area offsets are data driven", () => {
    ...
    const alcoholWeight = weights.find(
      (entry) => entry.typeId === "alcohol"
    ).weight;
