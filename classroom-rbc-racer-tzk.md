...
| 階段報告範本 | `PHASE_REPORT_TEMPLATE.md` |

3.4 版總案、循環術語基準、技術決策附錄、重建操作手冊、可執行測試與階段報告共同構成實作依據。本文件所有相對路徑均以 `rbc-racer/` Git 倉庫根目錄為準，不以文件所在目錄為準。Git 倉庫內同名文件是可交付正本；工作區上一層的同名文件是同步鏡像。
...
本案已完成 Phase 00～11，不再是尚待一次性產生的需求草稿。未來 AI、Codex 或開發者在空白電腦接手時，必須先判斷工作模式：

| 模式 | 使用時機 | 必須採取的動作 |
| --- | --- | --- |
| `RESTORE` | GitHub 可存取，目標是還原成品 | Clone、驗證、啟動；不得重寫既有系統 |
| `REBUILD` | 原始碼確實遺失、損壞，或使用者明確要求乾淨重建 | 在新的空目錄依 Phase 00～11 重建 |
| `MAINTAIN` | 完整專案已存在，目標是修正或擴充 | 先跑基線測試，再做最小變更與回歸 |

除非使用者明確要求 `REBUILD`，一律優先使用 `RESTORE`。不得因為看到第三十二章的階段流程，就在完成版 `main` 分支重新建立 Phase 00 或覆寫現有檔案。
...
3. `tests/`、`npm run test:phase11` 與瀏覽器實際行為。
4. `TECHNICAL_DECISIONS.md` 的已決策契約。
5. 本總案的玩法、醫學、架構與驗收規格。
6. `reports/` 的歷史證據與修正紀錄。
7. `codex-devp-cmd.md` 的操作流程。

術語基準優先權不授權改變玩法數值或內部英文 ID。程式碼與測試是已完成版本的可執行事實；本文件不得被用來合理化破壞已通過測試的改寫。若進行真正的 `REBUILD`，則以本總案、循環術語基準、技術決策附錄及階段測試門檻共同作為建置契約。
...
Phase 11 修正組織／肺氣體交換期間的小地圖節點，將一般減益的 Score 與 HP 扣分加倍，新增每 5 隻瘧原蟲觸發的血球破裂、每 10 次 CO 碰撞觸發的 CO 中毒、Time Out 肝臟工廠結局、程序化水蒸氣全畫面模糊、啟動可靠性、可見碰撞輪廓與玩家介面清理。Phase 11 是正式功能發布基線；Phase 10 仍是最近一次完整三瀏覽器與效能證據，不得把 Phase 11 的自動測試冒充為未執行的人工或效能驗收。
...
- 不以截圖或報告取代測試；每次重建或維護都必須實際執行 `npm run test:phase11`。
...
RBC Racer 是一款以人體血液循環為主題的第一人稱網頁賽車遊戲。
...
├─ reports/
│  ├─ README.md
│  ├─ phase-00-report.md … phase-11-report.md
...
└─ tests/
   ...
   ├─ unit-test.html
   ├─ unit/*.test.js
   ├─ phase-XX-manual-test-checklist.md
   ├─ phase-09-cutscene-preview.html
   └─ balance-test-notes.md
```

此樹狀圖是責任與必要檔案清單，不表示可以建立空白 placeholder。Clone 還原時以 Git tree 為準；乾淨重建時，每個檔案必須在所屬 Phase 具有可執行內容及對應測試。
...
本章程式區塊是設計契約摘要，不是 Phase 11 完整檔案。`RESTORE` 或 `MAINTAIN` 模式不得用本章片段覆蓋現有 `js/config.js`；完整數值以該檔及 `tests/phase-11-audit.mjs` 為準。`REBUILD` 模式則必須在 Phase 00 建立完整設定，再由各階段測試約束其演進。
...
不依賴外部測試框架。Node 入口為 `tests/run-tests.mjs`，瀏覽器入口為 `tests/unit-test.html`；Phase 11 基線必須維持 203 項測試及 9 項靜態稽核全部通過。至少涵蓋：
...
本章是原始開發歷程與真正 `REBUILD` 的分階段順序。對已 Clone 的完成版，Phase 00～11 報告均已 PASS；不得重新逐階段覆寫。維護工作應先執行完整基線，再以最小變更、回歸測試及新的變更報告處理。
...
7. 依 `PHASE_REPORT_TEMPLATE.md` 建立 `reports/phase-XX-report.md`。
...
- 確認本總案 3.0、`TECHNICAL_DECISIONS.md`、`codex-devp-cmd.md` 與 `PHASE_REPORT_TEMPLATE.md`
...
- 移除玩家畫面上的 Phase 標章、內部 state、FPS、Pointer Lock、checkpoint seed 與動畫 phase ID。
- `npm run test:phase11` 必須通過 203 tests 與 9 audits。
...
- `npm test` 為 203 passed、0 failed。
- `npm run test:audit` 為 9 passed、0 failed。
- `http://127.0.0.1:8000/` 顯示遊戲。
- `http://127.0.0.1:8000/tests/unit-test.html` 顯示 203 項通過。
...
2. 將本總案、`TECHNICAL_DECISIONS.md`、`codex-devp-cmd.md` 與報告範本放入新目錄。
3. 依 Phase 00～11 一次只執行一個階段。
4. 每階段先列授權範圍與排除項，再實作、測試、修正、重測及寫報告。
5. 前一階段不是 PASS 時，不得進入下一階段。
6. 不以歷史截圖或報告冒充執行證據；所有測試必須在新環境重跑。
7. Phase 11 完成後比較功能、測試數、vendor 雜湊、效能上限與 Pages 相對路徑。

詳細可直接交給 AI／Codex 的指令與階段矩陣位於 `codex-devp-cmd.md`。

## A.4 部署重建

`.github/workflows/deploy-pages.yml` 是正式部署入口，流程必須先執行 `npm run test:phase11`，通過後才上傳整個靜態站台並部署。新倉庫必須由擁有者啟用 GitHub Pages 的 GitHub Actions 來源；不得把私人 token、帳密或產生的部署憑證寫入專案。
