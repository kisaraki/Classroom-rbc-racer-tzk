...
| Esc 暫停端對端 | 鎖定後按 Esc | 位移停止，Real Clock／Renderer 繼續 | 使用者確認其餘行為正常，但舊版秒數看似靜止；新版待重測 | BLOCKED |
...
| Real Clock 狀態燈跳動但秒數看似靜止 | READY 即跳燈；30 秒後固定 `0.0` | READY 動畫 none；PAUSED 顯示 `T+1.0→T+2.0`、Renderer +30、world update +0 | 代碼關閉；桌面待重測 |
...
- Pointer Lock、滑鼠 yaw／pitch 與 Esc 解鎖已由使用者於桌面前景驗收；修正後 Real Clock 數字仍待同一環境重測。
